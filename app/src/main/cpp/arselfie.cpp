...
// in front of them.
    ...
    ArSelfie::ArSelfie(AAssetManager* asset_manager) : asset_manager_(asset_manager){
    ...
    void ArSelfie::OnResume(JNIEnv* env, void* context, void* activity) {
        LOGI("OnResume()");
    ...
    void ArSelfie::ChangeCameraDir(int32_t option) {
        ...
        ArCameraConfigList_getSize(ar_session_,configlist,&outsize);
        LOGI("outsize is %d at %s",outsize,__func__);
        ArCameraConfig_create(ar_session_,&cameraConfig);
        ArCameraConfigList_getItem(ar_session_,configlist,0,cameraConfig);
        ArSession_setCameraConfig(ar_session_,cameraConfig);
    ...
    void ArSelfie::OnSurfaceCreated() {
    ...
    void ArSelfie::OnDisplayGeometryChanged(int display_rotation,
    ...
    void ArSelfie::OnDrawFrame(bool depthColorVisualizationEnabled,
        ...
        if (ArSession_update(ar_session_, ar_frame_) != AR_SUCCESS) {
            LOGE("ArSelfie::OnDrawFrame ArSession_update error");
        }
    ...
    bool ArSelfie::IsDepthSupported() {
    ...
    void ArSelfie::ConfigureSession() {
    ...
    void ArSelfie::OnSettingsChange(bool is_instant_placement_enabled) {
    ...
    void ArSelfie::OnTouched(float x, float y) {
        if (ar_frame_ != nullptr && ar_session_ != nullptr) {
            ...
            if (ar_hit_result) {
                ...
                if (ArHitResult_acquireNewAnchor(ar_session_, ar_hit_result, &anchor) !=
                    AR_SUCCESS) {
                    LOGE(
                            "OnTouched ArHitResult_acquireNewAnchor error");
                    return;
                }
    ...
    void ArSelfie::UpdateAnchorColor(ColoredAnchor* colored_anchor) {
...
// the camera feed. It takes into account device orientation.
    glm::mat3 ArSelfie::GetTextureTransformMatrix(
